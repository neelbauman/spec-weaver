...
def repo_root(tmp_path):
    ...
    try:
        ...
        # SPEC ドキュメントの作成
        subprocess.run(
            ["doorstop", "create", "SPEC", "specs"], check=True, capture_output=True
        )
        ...
        item = (
            tree.find_item("SPEC1")
            or tree.find_item("SPEC-001")
            or tree.find_item("SPEC-1")
        )
        if not item:
...
""",
    ...
    result = runner.invoke(
        app, ["audit", str(feature_dir), "--repo-root", str(repo_root)]
    )
    ...
    # 2. review コマンドで更新
    result = runner.invoke(
        app, ["review", spec_id, "-f", str(feature_dir), "-r", str(repo_root)]
    )
...
""",
    ...
    # 5. status コマンドでの表示確認
    result = runner.invoke(
        app, ["status", "-f", str(feature_dir), "-r", str(repo_root)]
    )
    assert result.exit_code == 0
    assert "test-unreviewed" in result.stdout
...
def test_spec_change_makes_it_unreviewed_and_suspect_in_status(repo_root):
    ...
    (feature_dir / "test.feature").write_text(

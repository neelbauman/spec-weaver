...
#       - 単一アイテムをJSON形式で出力できる
#     """
#     raise NotImplementedError('STEP: 仕様アイテム "{param0}" が存在する')
...
@when('`spec-weaver semantic-review --all --output json` を実行する')  # type: ignore
def when_7ccd7bbd(context):
    """`spec-weaver semantic-review --all --output json` を実行する

    Scenarios:
      - 全体並列レビューが実行できる
    """
    raise NotImplementedError('STEP: `spec-weaver semantic-review --all --output json` を実行する')


@then('JSONが各アイテムのレビュー結果の配列またはディクショナリである')  # type: ignore
def then_61c6aa13(context):
    """JSONが各アイテムのレビュー結果の配列またはディクショナリである

    Scenarios:
      - 全体並列レビューが実行できる
    """
    raise NotImplementedError('STEP: JSONが各アイテムのレビュー結果の配列またはディクショナリである')
@given('claudeコマンドが利用可能である')  # type: ignore
def given_2b87969c(context):
    """claudeコマンドが利用可能である

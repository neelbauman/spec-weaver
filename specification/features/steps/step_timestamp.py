...
from _helpers import (
    PROJECT_ROOT, create_doorstop_project_api, minimal_feature,
    run_spec_weaver, write_feature_file,
)
...
def _git_commit_file(repo: Path, file_path: Path, message: str = "test commit") -> None:
    subprocess.run(["git", "add", str(file_path.relative_to(repo))],
...
def when_7e4b3813_fallback(context):
    ...
    context.updated_at = _get_git_file_date(str(context.yaml_file), mode="latest") or "-"

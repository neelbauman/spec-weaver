...
Feature: semantic-review コマンド — セマンティックレビュー
  ...
  Scenario: 単一アイテムのレビューが実行できる
    ...
    When `spec-weaver semantic-review --item SPEC-003 --feature-dir ./specification/features` を実行する
    Then 終了コードが0である
    And 出力にレビュー結果が含まれる
  ...
  Scenario: 単一アイテムをJSON形式で出力できる
    Given claudeコマンドが利用可能である
    And 仕様アイテム "SPEC-003" が存在する
    When `spec-weaver semantic-review --item SPEC-003 --output json` を実行する
    Then 終了コードが0である
    And 出力が有効なJSONである
  ...
  Scenario: 存在しないアイテムIDを指定するとエラーになる
    When `spec-weaver semantic-review --item NOTEXIST-999` を実行する
    Then 終了コードが1である
    And エラーメッセージが表示される

  Scenario: --item と --all は同時に指定できない
    When `spec-weaver semantic-review --item SPEC-003 --all` を実行する
    Then 終了コードが2である

  Scenario: --fail-on high でhigh findingがある場合に終了コード1を返す
    Given claudeコマンドが利用可能である
    And レビュー結果に severity "high" のfindingが含まれる
    When `spec-weaver semantic-review --item SPEC-003 --fail-on high` を実行する
    Then 終了コードが1である
  ...
  Scenario: --min-severity medium で low の finding が非表示になる
    Given claudeコマンドが利用可能である
    When `spec-weaver semantic-review --item SPEC-003 --min-severity medium` を実行する
    Then 終了コードが0である
